...
def main() :
    ...
    G = np.linspace(6e+10, 2 * 6e+10, 30)[::6]
    K = np.linspace(8e+10, 2 * 8e+10, 30)[::6]
    mesh_file = "tensile_test_specimen_special.msh"
    ...
    for i_g, g in enumerate(G) :
        for i_k, k in enumerate(K) :
            ...
            np.savez(f'/home/narupanta/ADDMM/surrogate_model/dataset/finer_field_{str(i_g)}_{str(i_k)}.npz', mesh_pos = coords[:, :2], node_connectivity = triangles, u = disp[:, :2])
